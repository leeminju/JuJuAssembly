...
//
//    //메서드의 동작을 mock으로 설정
//    when(chatRepository.findAllByRoomId(roomId)).th    when(userRepository.findUserByIdOrElseThrow(senderId)).thenReturn(sender);enReturn(fakeChats);
//
//
//
//    // 실제 메서드 호출
...
//    assertEquals("testuser", expect1.getMessage());
//    assertEquals(3L, expect1.getSenderId());
//
